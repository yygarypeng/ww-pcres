...
def load_cell_namespace(test_features, mean=None, scale=None):
    ...
    mean = np.arange(21, dtype=np.float64) / 10.0 if mean is None else mean
    scale = np.arange(1, 22, dtype=np.float64) / 10.0 if scale is None else scale
...
def test_uses_production_transform_checkpoint_stats_and_preserves_raw_inputs():
    ...
    np.testing.assert_allclose(namespace["checkpoint_mean"], np.arange(21) / 10.0)
    np.testing.assert_allclose(namespace["checkpoint_std"], np.arange(1, 22) / 10.0)
...
def test_jet_summaries_exclude_padding_and_scalar_summaries_use_all_events():
    raw = sample_features()
    _, _, namespace = load_cell_namespace(raw, mean=np.zeros(21), scale=np.ones(21))
...
def test_mixed_jet_histograms_use_dataset_fractions_and_include_padding_location():
    ...
    mean = np.zeros(21)
    mean[8:16] = 700.0
    scale = np.full(21, 100.0)

    _, _, namespace = load_cell_namespace(raw, mean=mean, scale=scale)
    gaussian_figure = plt.figure(plt.get_fignums()[-1])
...
def test_dphi_is_a_plain_scalar_diagnostic_without_unit_circle():
    _, _, namespace = load_cell_namespace(sample_features())
    input_figure = plt.figure(plt.get_fignums()[-1])

    assert len(input_figure.axes) == 21
    assert input_figure.axes[20].get_title().endswith("dphi_ll")
    assert input_figure.axes[20].lines
    assert input_figure.axes[20].patches
    assert "KS p-value" in namespace["summary_df"].columns
    assert "dphi_ll" in namespace["summary_df"].index
